...
class AnalyticsService:
    async def get_user_growth_trend(self, db: AsyncSession, days: int = 30) -> List[UserGrowthData]:
    ...
    async def get_detection_growth_trend(self, db: AsyncSession, days: int = 30) -> List[dict]:
    ...
    async def get_model_usage_statistics(self, db: AsyncSession) -> List[ModelUsageStat]:
        """Fetch CNN model usage logs grouped by name and version."""
        raw_stats = await dashboard_repository.get_model_usage_statistics(db)
        return [
            ModelUsageStat(model_name=item[0], model_version=item[1], count=item[2], average_confidence=item[3])

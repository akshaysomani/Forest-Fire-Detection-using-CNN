...
class ModelRegistryAdapter:
    @staticmethod
    async def get_checkpoint_by_run(
        db: AsyncSession,
        run_id: uuid.UUID,
        ...
        run_query = select(TrainingRun).where(
            and_(TrainingRun.id == run_id, TrainingRun.deleted_at.is_(None))
        )
    ...
    @staticmethod
    async def get_latest_completed_run_checkpoint(
        db: AsyncSession,

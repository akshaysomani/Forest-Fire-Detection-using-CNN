...
class ResponseTeamService:
    async def create_team(self, db: AsyncSession, name: str, specialty: str) -> ResponseTeam:
        ...
        team = ResponseTeam(name=name, specialty=specialty, status="Active")
        db.add(team)
        await db.flush()
        return team

    async def add_member_to_team(
        self, db: AsyncSession, team_id: uuid.UUID, user_id: uuid.UUID, role: str = "Responder"
    ) -> ResponseMember:
        ...
        dup_member_q = select(ResponseMember).where(
            and_(ResponseMember.team_id == team_id, ResponseMember.user_id == user_id, ResponseMember.deleted_at.is_(None))
        )
        res_dup = await db.execute(dup_member_q)
        if res_dup.scalar_one_or_none():
            raise ValidationException("User is already a member of this response team.")

        member = ResponseMember(team_id=team_id, user_id=user_id, role=role, is_available=True)
        db.add(member)
        await db.flush()
        return member

    async def set_member_availability(self, db: AsyncSession, member_id: uuid.UUID, is_available: bool) -> ResponseMember:

...
class ModelLifecycleService:
    ...
    async def transition_model_version(
        ...
        return await lifecycle_workflow_engine.trigger_transition(
            db=db, model_version_id=model_version_id, target_state=target_state, user_id=user_id, notes=notes
        )

    @staticmethod
    async def get_lifecycle_history(db: AsyncSession, model_version_id: uuid.UUID) -> List[ModelLifecycleEvent]:
        """Retrieves transition event history logs for a model version."""
        query = (
            select(ModelLifecycleEvent)
            .where(and_(ModelLifecycleEvent.model_version_id == model_version_id, ModelLifecycleEvent.deleted_at.is_(None)))

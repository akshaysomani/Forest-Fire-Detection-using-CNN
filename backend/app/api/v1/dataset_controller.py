...
async def list_datasets(
    ...
    total = await dataset_repository.count_datasets(
        db, category_id=category_id, status=status, search_query=search
    )
    return {
        "total": total,
        "skip": skip,
...
async def upload_single_file(
    ...
    return await dataset_upload_service.upload_single_file(
        db, dataset_id, file, current_user.id, label_id
    )
...
async def upload_bulk_files(
    ...
    return await dataset_upload_service.upload_bulk_files(
        db, dataset_id, files, current_user.id
    )
...
async def upload_zip_dataset(
    ...
    return await dataset_upload_service.upload_zip_dataset(
        db, dataset_id, file, current_user.id, background_tasks
    )
...
async def assign_labels(
    ...
    count = await label_manager.assign_label_to_files(
        db, id, payload.file_ids, payload.label_id, current_user.id
    )

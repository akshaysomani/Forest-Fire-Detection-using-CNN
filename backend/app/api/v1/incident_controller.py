...
async def create_manual_incident(
    ...
    """
    incident = await incident_service.create_manual_incident(
        db=db,
...
async def list_incident_audit_history(
    ...
    """
    items, total = await incident_repository.get_audit_history(
        db=db,
        skip=skip,
        limit=limit,
...
async def add_response_member(
    ...
    Requires 'manage_platform_settings' permission.
    """
    member = await response_team_service.add_member_to_team(
        db=db,
        team_id=id,
...
async def escalate_incident_manually(
    ...
    Requires 'view_alerts' permission.
    """
    incident = await escalation_manager.escalate_incident(
        db=db,
        incident_id=id,
        user_id=current_user.id,
        reason=body.reason
...
async def add_incident_update(
    ...
    event = IncidentEvent(
        ...
        payload={
            ...
            "message_snippet": body.message[:50]
        }
    )
    db.add(event)
    ...
    audit = IncidentAuditLog(
        incident_id=id,
        user_id=current_user.id,
        action="incident_update_created",
        details={

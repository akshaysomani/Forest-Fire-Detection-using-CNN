...
class TrainingRun(BaseModel):
    ...
    dataset_version_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("dataset_versions.id", ondelete="SET NULL"), nullable=True)
    status: Mapped[str] = mapped_column(String(50), default="pending", nullable=False)  # pending, running, completed, failed, stopped, resuming
    ...
    metrics_history: Mapped[dict | None] = mapped_column(JSON, nullable=True)  # List of epoch stats: [{"epoch": 1, "train_loss": 0.4, ...}]

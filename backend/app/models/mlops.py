...
class DeploymentJob(BaseModel):
    ...
    environment_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("environments.id", ondelete="CASCADE"), nullable=False)
    model_version_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, ForeignKey("model_versions.id", ondelete="CASCADE"), nullable=False
    )
    ...
    rollback_job_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        Uuid, ForeignKey("deployment_jobs.id", ondelete="SET NULL"), nullable=True
    )
    deployed_by: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)

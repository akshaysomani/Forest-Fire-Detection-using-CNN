...
class Incident(BaseModel):
    ...
    status: Mapped[str] = mapped_column(String(20), default="Open", nullable=False, index=True)  # Open, Acknowledged, Assigned, In Progress, Escalated, Resolved, Closed
    severity: Mapped[str] = mapped_column(String(20), default="Medium", nullable=False, index=True)  # Critical, High, Medium, Low, Informational
    ...
    updates: Mapped[List["IncidentUpdate"]] = relationship("IncidentUpdate", back_populates="incident", cascade="all, delete-orphan")
    status_history: Mapped[List["IncidentStatusHistory"]] = relationship("IncidentStatusHistory", back_populates="incident", cascade="all, delete-orphan")
    audit_logs: Mapped[List["IncidentAuditLog"]] = relationship("IncidentAuditLog", back_populates="incident", cascade="all, delete-orphan")
...
class ResponseTeam(BaseModel):
    ...
    # Relationships
    members: Mapped[List["ResponseMember"]] = relationship("ResponseMember", back_populates="team", cascade="all, delete-orphan")
    assignments: Mapped[List["IncidentAssignment"]] = relationship("IncidentAssignment", back_populates="team", cascade="all, delete-orphan")
...
class ResponseMember(BaseModel):
    ...
    team_id: Mapped[uuid.UUID] = mapped_column(
        Uuid,
        ForeignKey("response_teams.id", ondelete="CASCADE"),
        nullable=False,
        index=True
    )
...
class IncidentAssignment(BaseModel):
    ...
    status: Mapped[str] = mapped_column(String(20), default="Pending", nullable=False, index=True)  # Pending, Accepted, Rejected, Completed
